############### 文件无用,只是在SGD优化器时,用来进行学习率调整的验证,可以完全不管###############
...
class FindLR(_LRScheduler):
    ...
    Args:
        ...
        num_iter: totoal_iters 设置的迭代出次数
...
if __name__ == '__main__':
    ...
    epoches = int(args.num_iter / len(imagenet_training_loader)) + 1
    ...
    for epoch in range(epoches):
        ...
        for batch_index, (images, labels) in enumerate(imagenet_training_loader):
            ...
            lr_scheduler.step()     # 训练的时候进行学习率规划,更新参数
            ...
            if torch.isnan(loss).any(): #避免训练过程中loss出现为Nan的情况;(可以通过加上一个很小的余项来进行避免)
                n += 1e8                # any() 函数用于判断给定的可迭代参数 iterable 是否全部为 False，则返回 False，如果有一个为 True，则返回 True。
                break
            loss.backward()
            optimizer.step()
    ...
    learning_rate = learning_rate[10:-5]    #[10:-5] 表示:舍弃列表前10个和最后5个元素
    losses = losses[10:-5]
